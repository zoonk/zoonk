defmodule Zoonk.AI.AISchemaTest do
  ...
  describe "add_field/2" do
    ...
    test "supports array as the first element" do
      ai_schema = AISchema.add_field(%AISchema{}, [%{title: "string", description: "string"}])

      assert ai_schema.type == "json_schema"
      assert ai_schema.name == ""
      assert ai_schema.strict == true

      assert ai_schema.schema.type == "array"
      assert ai_schema.schema.additionalProperties == false

      assert ai_schema.schema.items.type == "object"
      assert ai_schema.schema.items.required == ["description", "title"]
      assert ai_schema.schema.items.additionalProperties == false

      assert ai_schema.schema.items.properties.title.type == "string"
      assert ai_schema.schema.items.properties.description.type == "string"
    end
  end
end
