...
import { languageStoryActivityWorkflow } from "./kinds/language-story-workflow";
...
export async function languageActivityWorkflow(
  ...
  const [vocabularyResult] = await Promise.allSettled([
    ...
    languageStoryActivityWorkflow(activities, workflowRunId, concepts, neighboringConcepts),
  ]);
