...
import { languagePracticeActivityWorkflow } from "./kinds/language-practice-workflow";
...
export async function languageActivityWorkflow(
  ...
  const [vocabularyResult] = await Promise.allSettled([
    ...
    languagePracticeActivityWorkflow(activities, workflowRunId, concepts, neighboringConcepts),
  ]);
