...
test.describe("Course Chapters Accordion", () => {
  test("displays chapters with position numbers and expands to show lessons", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Data Preparation/i })).toBeVisible();
    await expect(page.getByRole("button", { name: /Regression Algorithms/i })).toBeVisible();

    // Click to expand the first chapter
    await page.getByRole("button", { name: /Introduction to Machine Learning/i }).click();
  ...
  test("closes current chapter when another is opened", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /Understanding Datasets/i })).toBeVisible();
    await expect(page.getByRole("link", { name: /Data Cleaning/i })).toBeVisible();
...
test.describe("Course Chapters - Empty State", () => {
  test("renders course page without chapters gracefully", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Python Fundamentals/i })).toBeVisible();
  });
});

test.describe("Course Chapters - No Lessons", () => {
  test("shows generate lessons message and navigates to generate page", async ({ page }) => {
    await page.goto("/b/ai/c/python-programming");
    ...
    await expect(page.getByRole("heading", { name: /generate chapter/i })).toBeVisible();
  });
});
...
test.describe("Course Chapters - Locale", () => {
  test("shows chapters in Portuguese for Portuguese locale", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Preparação de Dados/i })).toBeVisible();
  });
});
...
test.describe("Course Chapter Search", () => {
  test("filters chapters by chapter title", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Data Preparation/i })).not.toBeVisible();
  });

  test("filters by lesson title and auto-expands parent chapter", async ({ page }) => {
    await page.goto("/b/ai/c/machine-learning");
    ...
    await expect(page.getByRole("link", { name: /Understanding Datasets/i })).not.toBeVisible();
  });
  ...
  test("matches Portuguese chapters without accents (accent-insensitive search)", async ({
    ...
    // Non-matching chapters should be hidden
    await expect(page.getByRole("button", { name: /Preparação de Dados/i })).not.toBeVisible();
  });
});
