...
type ProgressChartFigureProps = { children: ReactNode; label: string };

type ProgressChartTooltipProps<DataPoint> = { children: (data: DataPoint) => ReactNode };

type ProgressChartGradientProps = { color: string; id: string };
...
const PROGRESS_CHART_STYLE = { height: 256, width: "100%" } satisfies CSSProperties;

/**
 * Gives each progress chart the same accessible container and flex-safe
 ...
 * remember it.
 */
export function ProgressChartFigure({ children, label }: ProgressChartFigureProps) {
  return (
...
/**
 * Keeps every progress AreaChart on the same Recharts sizing path.
...
/**
 * Keeps chart background grid styling consistent across every progress
...
/**
 * Keeps every progress chart on the same x-axis contract. All chart data is
 ...
 */
export function ProgressChartXAxis() {
  return (
    <XAxis
      axisLine={false}
      dataKey="label"
 ...
 */
export function ProgressChartPercentYAxis() {
  return (
    <YAxis
      axisLine={false}
      domain={[0, 100]}
 ...
 * Every progress chart uses the same fade shape, and the caller only decides
 * which CSS color token owns that metric.
 */
export function ProgressChartGradient({ color, id }: ProgressChartGradientProps) {
  return (
 ...
 */
export function ProgressChartArea({ color, dataKey, gradientId }: ProgressChartAreaProps) {
  return (
    <Area
      dataKey={dataKey}
      fill={`url(#${gradientId})`}
...
/**
 * Handles Recharts' tooltip payload guard once for all progress charts.
 ...
 * metric chart.
 */
export function ProgressChartTooltip<DataPoint>({
  children,
}: ProgressChartTooltipProps<DataPoint>) {
  return (
    <Tooltip
...
/**
 * Keeps tooltip containers visually identical across progress charts. The
 ...
 */
export function ProgressChartTooltipContent({ children }: { children: ReactNode }) {
  return <div className="bg-background rounded-lg border px-3 py-2 shadow-sm">{children}</div>;
}

/**
 * Keeps tooltip date labels muted and compact across every progress chart.
 ...
 */
export function ProgressChartTooltipLabel({ children }: { children: ReactNode }) {
 ...
 */
export function ProgressChartTooltipValue({ children, className }: ComponentProps<"p">) {
  return <p className={cn("text-sm font-medium", className)}>{children}</p>;
}

/**
 * Keeps the average marker identical for percent-based progress charts.
 ...
 */
export function ProgressChartAverageLine({ children, y }: ProgressChartAverageLineProps) {
  return (
    <ReferenceLine
      label={{
