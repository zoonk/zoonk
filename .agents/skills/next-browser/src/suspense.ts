...
function buildBoundaryInsight(shell: Boundary, resolved: Boundary): BoundaryInsight {
...
function collectFilesToRead(holes: BoundaryInsight[], rootCauses: RootCauseGroup[]): string[] {
...
function inferBoundaryKind(boundary: Boundary): BoundaryKind {
  ...
  if (boundary.name?.includes("Suspense") || ownerNames.some((name) => name.includes("Suspense"))) {
    return "explicit-suspense";
  }
  return "component";
}

function classifyBlocker(suspender: Suspender, sourceFrame: StackFrame | null): BlockerKind {
...
function buildBlockerKey(name: string, kind: BlockerKind, sourceFrame: StackFrame | null): string {
...
async function resolveStack(stack: StackFrame[], origin: string): Promise<StackFrame[]> {
  ...
  function parseInspection(boundary: Boundary, data: any) {
    ...
    if (Array.isArray(data.owners)) {
      for (const o of data.owners) {
        if (o?.displayName) {
          const src =
            Array.isArray(o.stack) && o.stack.length > 0 && Array.isArray(o.stack[0])
              ? ([o.stack[0][1] || "(unknown)", o.stack[0][2], o.stack[0][3]] as [
                  string,
                  number,
                  number,
                ])
