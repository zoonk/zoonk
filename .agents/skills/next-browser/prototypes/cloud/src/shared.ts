 ...
 */
export async function installChrome(sandbox: Sandbox, cwd = "/vercel/sandbox"): Promise<string> {
  ...
  const install = await shell(sandbox, `cd ${cwd} && pnpm add @sparticuz/chromium puppeteer-core`);
 ...
 */
export async function fixTurbopackRoot(sandbox: Sandbox, appDir: string): Promise<void> {
