  ...
  for (const scanner of scanners) {
    try {
      ...
      for (const f of found ?? []) {
        findings.push({ ...f, route: mapFileToRoute(f.file, routes) });
      }
    } catch (err) {
      process.stderr.write(
...
async function enumerateRoutes(root) {
  ...
  for (const e of entries) {
    ...
    if (!m) {
      ...
      if (top) {
        routes.push({ routePath: "/", file: rel, type: routeEntryType(top[1]) });
        continue;
      }
    }
    if (m) {
      ...
      routes.push({ routePath, file: rel, type: routeEntryType(m[2]) });
      continue;
    }
    ...
    if (m) {
      ...
      routes.push({ routePath: name === "" ? "/" : "/" + name, file: rel, type: "route" });
      continue;
    }
    ...
    if (m) {
      const name = normalizeRouteFileStem(m[1]);
      routes.push({ routePath: name === "" ? "/" : "/" + name, file: rel, type: "page" });
      continue;
    }
