...
async function getTeamInfo(teamIdOrSlug) {
  ...
  return { ok: true, id: team.id ?? null, slug: team.slug ?? null, name: team.name ?? null };
}
...
export async function runVercelJson(args, opts = {}) {
  ...
  return { ok: false, code: categorizeError(exitCode, stderr), stderr: safeStderr };
}
...
export function filterUsageByProject(usage, projectId, projectName = null) {
  ...
  const out = { ...usage, breakdown: { ...breakdown, data: [] } };
...
async function getBillingPlanFromPath(path, source) {
  ...
  if (!parsed) {
    return { plan: "unknown", reason: `${source} missing from Vercel API response`, source };
  }

  return { ...parsed, reason: `${source}=${parsed.plan}`, source };
}
