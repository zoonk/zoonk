...
export function extractClaims(rec, ctx = {}) {
  ...
  if (cacheFiles.length > 0) {
    ...
    claims.push({ type: "cache_control_header_syntax", rec, sourceField: "cache-header-syntax" });
    claims.push({
    ...
    if (mentionsCachedNotFoundOr404(rec)) {
      claims.push({ type: "cache_404_long_ttl_safety", rec, sourceField: "cache-404-safety" });
    }
  }
  ...
  if (mentionsRuntimeErrorCause(rec)) {
    claims.push({ type: "runtime_error_cause_supported", rec, sourceField: "runtime-error-cause" });
  }
