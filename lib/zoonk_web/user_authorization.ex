defmodule ZoonkWeb.UserAuthorization do
  ...
  ## `on_mount` arguments
    ...
    * `:ensure_catalog_access` - Verifies that the organization has access to the catalog feature.
      If not, raises a `PermissionError` with code `:feature_disabled`.
    * `:ensure_system_org` - Verifies that the organization is a system organization.
      If not, raises a `PermissionError`.
  ...
  def on_mount(:ensure_system_org, _params, _session, socket) when socket.assigns.scope.org.kind == :system do
    {:cont, socket}
  end

  def on_mount(:ensure_system_org, _params, _session, _socket) do
