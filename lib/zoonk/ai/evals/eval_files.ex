defmodule Zoonk.AI.Evals.EvalFiles do
  ...
  """
  def store_output(output_type, model, prompt, results_dir, filename, data) do
    Logger.info("Storing #{output_type} results for #{model} in #{filename}")
  ...
  ## Examples
      ...
      false
  """
  def file_exists?(output_type, model, prompt, results_dir, filename) do
    output_type
    |> output_dir!(model, prompt, results_dir)
    |> Path.join(filename)
    |> File.exists?()
