...
type CompletionProgress = Pick<CompletionResult, "brainPower" | "newTotalBp">;
...
export type PlayerCompletionMilestone = LevelCompletionMilestone;
 ...
 *
 * The returned array is intentionally generic even though today it only
 * contains level milestones. Future completion milestones can append their own
 * entries here without changing the reducer or screen-routing shape.
 */
...
export function getInitialCompletionMilestoneIndex({
  ...
  return getCompletionMilestones({ completion, previousTotalBrainPower }).length > 0 ? 0 : null;
}
