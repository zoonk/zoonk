...
import { getLesson } from "./get-lesson-by-slug";
...
describe(getLesson, () => {
  ...
  it("returns lesson with chapter and course info", async () => {
    ...
    expect(cacheTag).toHaveBeenCalledWith(
      getCourseCacheTag(publishedCourse.id),
      getChapterCacheTag(publishedChapter.id),
      getLessonCacheTag(publishedLesson.id),
    );
  });

  it("normalizes an encoded Unicode lesson slug", async () => {
  ...
  it("returns null for lesson in unpublished chapter", async () => {
  ...
  it("returns null for lesson in unpublished course", async () => {
  ...
  it("returns null for lesson from non-brand org", async () => {
