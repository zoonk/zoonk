...
/**
 * Exercises the public completion boundary with an authenticated fixture user
 * while keeping each test's identity explicit.
 */
async function submitCompletionForUser({
  input,
  userId,
}: {
...
const inaccessibleLessonCases: { name: string; visibility: CompletableLessonVisibility }[] = [
  ...
  { name: "generated lesson", visibility: { lessonGenerationStatus: "pending" } },
...
async function createMultipleChoiceLesson(params: {
  ...
  const lesson = await lessonFixture({
    ...
    generationStatus: params.generationStatus ?? "completed",
...
describe(completeLesson, () => {
  ...
  it.each(inaccessibleLessonCases)(
    ...
    async (testCase) => {
      ...
      const { lesson, step } = await createMultipleChoiceLesson({
        ...
        generationStatus: testCase.visibility.lessonGenerationStatus,
        isPublished: testCase.visibility.lessonIsPublished,
        organizationId: context.organization.id,
      });

      await submitCompletionForUser({
  ...
  it("does not write completion progress for a guest", async () => {
    ...
    await expect(
      completeLesson(buildCompletionInput({ lessonId: lesson.id, stepId: step.id })),
    ).resolves.toStrictEqual({ status: "unauthorized" });

    await expect(
      prisma.lessonProgress.findMany({ where: { lessonId: lesson.id } }),
