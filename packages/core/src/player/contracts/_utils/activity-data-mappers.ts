export type ActivityPronunciationInput = {
...
export type ActivityWordInput = {
  ...
  pronunciations: ActivityPronunciationInput[];
};
...
export type ActivityStepInput = {
  ...
  kind: string;
  position: number;
  word: Omit<ActivityWordInput, "pronunciations"> | null;
  sentence: ActivitySentenceInput | null;
};

/**
 * Maps a lesson-scoped word row to the flat player input shape.
 *
 * `prepareActivityData` should only receive plain serializable data, not database-shaped
...
/**
 * Maps lesson-scoped vocabulary rows to the flat shape consumed by `prepareActivityData`.
 */
...
/**
 * Maps lesson-scoped sentence rows to the flat shape consumed by `prepareActivityData`.
 */
 ...
 */
function toDistractorWordInput(word: ActivityDistractorWordInput): DistractorWordDataInput {
  return {
