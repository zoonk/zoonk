...
async function getPrivateCourse(courseSlug: string) {
  "use cache: private";
  ...
  const course = await prisma.course.findFirst({
    include: { categories: true, organization: true },
    where: { isPublished: true, organizationId: null, slug: courseSlug, userId: session.user.id },
  });
...
export async function getCourse(params: { brandSlug: string; courseSlug: string }) {
  if (decodeRouteParam(params.brandSlug) === "me") {
    return getPrivateCourse(decodeRouteParam(params.courseSlug));
  }
