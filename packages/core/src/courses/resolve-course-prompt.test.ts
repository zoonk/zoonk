...
/**
 * Narrows direct-course results so tests can assert the reusable domain
 * resource without weakening the resolver union type.
 */
...
describe("course-prompt", () => {
  it.each([
    ...
    { expectedReason: "prompt", language: "en", prompt: "   " },
    {
      ...
      language: "a".repeat(COURSE_LANGUAGE_MAX_LENGTH + 1),
      prompt: "biology",
    },
  ] as const)(
    "rejects invalid $expectedReason input before calling models or persistence",
    async ({ expectedReason, language, prompt }) => {
      const courseFormatSpy = vi.spyOn(formatTask, "classifyCourseFormat");
      const intentSpy = vi.spyOn(intentTask, "classifyCourseIntent");
      const personalizationSpy = vi.spyOn(personalizationTask, "classifyCoursePersonalization");
      const titleSpy = vi.spyOn(canonicalTitleTask, "generateCanonicalCourseTitle");

      await expect(resolveCoursePrompt({ language, prompt })).resolves.toStrictEqual({
        kind: "invalid",
      ...
      await expect(
        prisma.coursePrompt.count({
          where: { language, normalizedPrompt: normalizeString(prompt) },
        }),
      ).resolves.toBe(0);
    },
  );
  ...
  it("persists and redirects reusable language prompts without target language", async () => {
    ...
    expect(result).toStrictEqual({ kind: "language" });
    expect(intentSpy).toHaveBeenCalledOnce();
    expect(personalizationSpy).toHaveBeenCalledOnce();
    expect(courseFormatSpy).toHaveBeenCalledOnce();
    expect(titleSpy).toHaveBeenCalledOnce();
  ...
  it("persists and redirects exam prompts to the exam start path", async () => {
    ...
    expect(result).toStrictEqual({ kind: "exam" });
    expect(intentSpy).toHaveBeenCalledOnce();
    expect(personalizationSpy).toHaveBeenCalledOnce();
    expect(courseFormatSpy).toHaveBeenCalledOnce();
    expect(titleSpy).toHaveBeenCalledOnce();
