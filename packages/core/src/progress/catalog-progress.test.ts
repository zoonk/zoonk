...
import { getChapterProgress } from "./get-chapter-progress-for-current-user";
import { getChapterContinueProgress, getCourseContinueProgress } from "./get-continue-progress";
...
/** Selects the shared fixture learner as the authenticated request identity. */
function getAuthenticatedCatalogFixtures() {
  mockSession(catalogContext.user.id);
...
describe("catalog progress data", () => {
  it("returns lesson progress rows for the chapter grid", async () => {
    const { organization, user } = getAuthenticatedCatalogFixtures();
    ...
    await expect(getChapterProgress({ chapterId: chapter1.id })).resolves.toStrictEqual([
  ...
  it("returns chapter progress rows for the course grid", async () => {
    ...
    await expect(getCourseProgress({ courseId: course.id })).resolves.toStrictEqual([
  ...
  it("applies hidden lesson-kind filters across catalog and continue progress", async () => {
    ...
    await expect(
      getChapterProgress({ chapterId: chapter1.id, excludedLessonKinds: ["quiz"] }),
    ).resolves.toStrictEqual([{ isCompleted: true, lessonId: visibleLesson.id }]);

    await expect(
      getCourseProgress({ courseId: course.id, excludedLessonKinds: ["quiz"] }),
  ...
  it("does not expose learner progress to guests", async () => {
    const { organization, user } = catalogContext;

    const { chapter1, course } = await createPublishedCourseChapters({
      organizationId: organization.id,
    });

    const [lesson] = await createPublishedLessons({
      chapterId: chapter1.id,
      count: 1,
      organizationId: organization.id,
    });
    ...
    if (!lesson) {
      throw new Error("Expected a lesson fixture");
    }

    await completeLessons({ lessonIds: [lesson.id], userId: user.id });

    mockSession(null);

    const [chapterProgress, courseProgress] = await Promise.all([
      getChapterProgress({ chapterId: chapter1.id }),
  ...
  it("isolates completion rows between authenticated learners", async () => {
    const { organization, user } = catalogContext;
    ...
    await expect(getChapterProgress({ chapterId: chapter1.id })).resolves.toStrictEqual([
      { isCompleted: false, lessonId: lesson.id },
    ]);
  });
});
