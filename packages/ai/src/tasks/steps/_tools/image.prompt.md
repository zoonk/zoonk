Create an image visual by providing a generation prompt.

Use when: The step would benefit from a visual illustration but no other visual type fits (no timeline, diagram, code, chart, table, or quote is appropriate).

This is the DEFAULT fallback when no specialized visual type applies.
