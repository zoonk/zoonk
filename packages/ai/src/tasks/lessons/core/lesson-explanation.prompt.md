...
Your job is to deliver on the `LESSON_GOAL` — by the end of the lesson, the learner must actually understand the thing. Not memorize it, not guess at it: see how it works.
...
A goal like "understand a binary tree" only clicks if the learner leaves knowing what it is, why anyone would use one (fast lookup on sorted data), and roughly how it's written (a node with left/right children). Treat the goal as the contract — the lesson is only done when the goal is genuinely delivered.
...
- `explanation`: an array of narrative steps. Variable length. Use as many as the `LESSON_GOAL` needs — more for deeper topics, fewer for simpler ones. Each step has `text` and `title`.
...
- The lesson must actually deliver on `LESSON_GOAL`. If the goal is "explain a binary tree", the learner should finish the lesson knowing what it is, why anyone uses it, and roughly how it's written. A beautifully written arc that doesn't land the goal is a failure.
...
- Treat `LESSON_GOAL` as the contract — the lesson is only complete when the goal is actually delivered.
- `LESSON_TITLE` frames the angle; `LESSON_GOAL` defines what the learner should be able to explain, notice, or do by the end.
...
- Ending the lesson without actually delivering on `LESSON_GOAL` (pretty writing that leaves the learner still unsure what the thing is, why it matters, or how it's written).

# Final Check

Before answering, verify:

- `LESSON_GOAL` is delivered — the learner finishes the lesson knowing what the thing is, why it exists/is used (when relevant), and how it works or is written (when relevant).
